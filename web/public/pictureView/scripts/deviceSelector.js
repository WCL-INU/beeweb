...
async function loadHives(areaId) {
    ...
    if (areaId) {
        ...
        const hiveData = await fetchData(`api/hive?areaId=${areaId}`);
...
async function loadDevices(hiveId) {
    ...
    if (hiveId) {
        ...
        const deviceData = await fetchData(`api/device?hiveId=${hiveId}`);
...
document.addEventListener('DOMContentLoaded', async function () {
    deviceTypes = await fetchData('api/devicetypes');
    const areaData = await fetchData('api/area');
