...
async function fetchInOutData(deviceId, sTime, eTime) {
  ...
  if (!response.ok) {
    console.warn(
      `No InOut data for device ${deviceId} (status: ${response.status})`
    );
    return []; // 데이터 없음 처리
  }
...
async function fetchSensorData(deviceId, sTime, eTime) {
  ...
  if (!response.ok) {
    console.warn(
      `No Sensor data for device ${deviceId} (status: ${response.status})`
    );
    return []; // 데이터 없음 처리
  }
...
async function fetchDataList() {
  ...
  for (const device of fetcher_deviceList) {
    ...
    if (device.type_id == 2) {
      ...
      const humiData = data.map((d) => ({
        id: d.id,
        value: d.humi,
        time: d.time,
      }));
      const co2Data = data.map((d) => ({
        id: d.id,
        value: d.co2,
        time: d.time,
      }));
...
document.addEventListener("timeRangeUpdated", async (event) => {
  console.log(
    `Time range updated: ${event.detail.sTime} ~ ${event.detail.eTime}`
  );
