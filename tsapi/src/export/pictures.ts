...
const zipFile = async (filePath: string, outPath: string): Promise<void> => {
    await new Promise<void>((resolve, reject) => {
        const output = fs.createWriteStream(outPath);
        const archive = archiver("zip", { zlib: { level: 9 } });
...
export const exportPictures = async (
    ...
    try {
        ...
        const innerZipPath = path.join(workDir, "pictures.zip");
        await zipDirectory(workDir, innerZipPath);

        await zipFile(innerZipPath, outerZipPath);
